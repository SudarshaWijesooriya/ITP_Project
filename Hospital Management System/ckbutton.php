...
if ( $result -> num_rows>0)
	{
		...
		{	
			...
			/*$sql= "SELECT r.Quantity,r.Item_Name,i.Name,i.Current_Stock from requestlist r,item i 
	...
	echo $nmi;*/
	$result = mysqli_query($con,"SELECT * FROM item WHERE Name='$nm'");
if ( $result -> num_rows>0)
	{
		while ($row = $result -> fetch_assoc())
		{
			
			if($qyt<=$row['Current_Stock']){
				$q=$row['Current_Stock']-$qyt;
				if($q>=$row['Min_Stock']){
					
					$sq="UPDATE item set Current_Stock=Current_Stock-$qyt WHERE Name='$nm'";
					if ($con->query($sq) == TRUE) {
					  
					  $sqll = "DELETE FROM requestlist WHERE RequestListID='" . $_GET["id"] . "'";
							if (mysqli_query($con, $sqll)) {
								echo '<script>alert("Item can issue")</script>';
								echo '<script>alert("Record updated successfully and deleted successfully")</script>';
					...
					echo '<script>alert("Item can not issue")</script>';
					header('location:cannotissue.php');
				}
		...
		echo '<script>alert("Item not in the stocks")</script>';
	}
